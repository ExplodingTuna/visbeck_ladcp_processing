...
while (~strncmp(str,'*END*',5));
    ...
    elseif (strncmp(str,'# name',6))  
        ...
        ctd.varunits{var}=str(findstr(str,':')+1:end);
    ...
    %------------------------------
    %
    %    Read the sensor calibration coefficients into a cell array
    %
    elseif any(findstr(str,'Oxygen, SBE 43 -->'))
        str=fgetl(fid);
        str=fgetl(fid);
        % contains serial number between first ">" and second ">"        
        ox(1).serial = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%d');
        str=fgetl(fid); str=fgetl(fid); str=fgetl(fid); str=fgetl(fid);
        str=fgetl(fid); str=fgetl(fid); str=fgetl(fid); str=fgetl(fid);
        str=fgetl(fid); str=fgetl(fid); str=fgetl(fid); str=fgetl(fid);
        str=fgetl(fid); str=fgetl(fid);
        % contains SOC.  Following lines are coeffiecients for new Seabird
        ox(1).Soc = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).offset = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).A = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).B = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).C = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).D0 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).D1 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).D2 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).E = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).Tau20 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).H1 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).H2 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(1).H3 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
    elseif any(findstr(str,'Oxygen, SBE 43, 2 -->'))
        str=fgetl(fid);
        str=fgetl(fid);
        % contains serial number between first ">" and second ">"        
        ox(2).serial = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%d');
        str=fgetl(fid); str=fgetl(fid); str=fgetl(fid); str=fgetl(fid);
        str=fgetl(fid); str=fgetl(fid); str=fgetl(fid); str=fgetl(fid);
        str=fgetl(fid); str=fgetl(fid); str=fgetl(fid); str=fgetl(fid);
        str=fgetl(fid); str=fgetl(fid);
        % contains SOC.  Following lines are coeffiecients for new Seabird
        ox(2).Soc = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).offset = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).A = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).B = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).C = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).D0 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).D1 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).D2 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).E = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).Tau20 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).H1 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).H2 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
        str=fgetl(fid);
        ox(2).H3 = sscanf(str(min(findstr(str,'>'))+1:max(findstr(str,'<'))-1),'%f');
    end
end
